...
pub struct GGUFHeader {
    ...
    pub metadata: BTreeMap<String, GGUFMetadata>,
}
...
#[derive(PartialEq, Debug, serde::Serialize, Clone)]
pub struct GGUFTensor<T> {
    pub name: String,
...
#[derive(PartialEq, serde::Serialize, Debug)]
pub struct GGUFFile<T> {
    pub header: GGUFHeader,
    pub tensors: BTreeMap<String, GGUFTensor<T>>,
...
/// GGUF metadata
#[derive(PartialEq, serde::Serialize, Debug, Clone)]
...
pub fn header_find_string_array<'a>(
    header: &'a GGUFHeader,
    key: &str,
) -> Result<Vec<&'a String>, anyhow::Error> {
    match header.metadata.get(key) {
        Some(v) => match &v.value {
            GGUFMetadataValue::Array(a) => {
                let x = a
                    .value
                    .iter()
                    .map(|v| match v {
                        GGUFMetadataValue::String(s) => s, // TODO: do not clone
                        _ => unimplemented!(),
                    })
                    .collect();
                Ok(x)
            }
            _ => Err(anyhow::anyhow!("Can't convert metadata '{}' to array", key).into()),
        },
        None => Err(anyhow::anyhow!("Could not find {} in metadata", key)),
    }
}

pub fn header_find_string<'a>(header: &'a GGUFHeader, key: &str) -> Result<&'a str, anyhow::Error> {
    match header.metadata.get(key) {
        Some(v) => match &v.value {
            GGUFMetadataValue::String(s) => Ok(s),
            _ => Err(anyhow::anyhow!("Can't convert metadata '{}' to string", key).into()),
        },
        None => Err(anyhow::anyhow!("Could not find {} in metadata", key)),
    }
}

pub fn header_find_usize<'a>(header: &'a GGUFHeader, key: &str) -> Result<usize, anyhow::Error> {
    match header.metadata.get(key) {
        Some(v) => match &v.value {
            GGUFMetadataValue::Uint64(u) => Ok(*u as usize),
            GGUFMetadataValue::Uint32(u) => Ok(*u as usize),
            _ => {
                Err(anyhow::anyhow!("Can't convert metadata '{}' to positive integer", key).into())
            }
        },
        None => Err(anyhow::anyhow!("Could not find {} in metadata", key)),
    }
...
pub fn header_find_f32<'a>(header: &'a GGUFHeader, key: &str) -> Result<f32, anyhow::Error> {
    match header.metadata.get(key) {
        Some(v) => match &v.value {
            GGUFMetadataValue::Float32(v) => Ok(*v),
            _ => Err(anyhow::anyhow!("Can't convert metadata '{}' to 32-bits float", key).into()),
        },
        None => Err(anyhow::anyhow!("Could not find {} in metadata", key)),
    }
}
